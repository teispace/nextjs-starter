...
- **Authorization**: `AuthUser` carries optional `roles` / `permissions`; guard pages with `requireRole` / `requirePermission` (they call `forbidden()`), hide controls with `hasRole` / `hasPermission`, and declare an action's claims in `.metadata({ name, permissions })`. Checks fail closed and are never the enforcement point: the API is.
- **Rate limiting**: `rateLimit` + `callerKey` + `tooManyRequests` from `@/lib/security`. The refresh route and the BFF proxy are limited; the default store is per process, so pass a shared `RateLimitStore` in production.
