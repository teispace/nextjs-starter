...
- **HTTP**: one transport-agnostic client with pluggable adapters (`fetch` default, `axios` optional), bounded retries with jitter and `Retry-After`, timeouts, cancellation, request-id correlation, envelope unwrapping, optional zod validation, and `HttpError` with field-level helpers. Universal, server (cookie-forwarding), and public-server entries, plus an optional same-origin proxy route (BFF) so the browser never talks to the API directly. See [src/lib/http/README.md](src/lib/http/README.md).
...
- **SEO**: `generateSEOMetadata` with canonical and hreflang, generated Open Graph card, manifest, generated icons, viewport theme colours, JSON-LD helpers, sitemap and robots. Section-level error boundaries built on `catchError` keep one failing section from taking down a page.
