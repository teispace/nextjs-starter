...
describe('WsClient', () => {
  ...
  describe('connect()', () => {
    ...
    it('refuses when a custom auth provider yields nothing and emits AUTH_REQUIRED', async () => {
      const client = new WsClient({ auth: async () => undefined });
    ...
    it('allows anonymous connect even when the auth provider yields nothing', async () => {
      const client = new WsClient({ auth: async () => undefined });
